...
def soft_dice_score(
    ...
    assert output.size() == target.size(), f"Output size {output.size()} is not equal to target size {target.size()}"
    if dims is not None:
        intersection = torch.sum(output * target, dim=dims)
        cardinality = torch.sum(output ** exponent + target ** exponent, dim=dims)
    else:
        intersection = torch.sum(output * target)
        cardinality = torch.sum(output ** exponent + target ** exponent)
