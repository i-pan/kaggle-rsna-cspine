...
def apply_mosaic_3d(X, y, center_ratio_range=(0.25, 0.75)):
    """
    Implementation of Mosaic augmentation for 3D segmentation. 
    ...
    **Note: batch size PER PROCESS must be at least 4
    ...
    """
    B, Z, C, H, W = X.size()
    assert H == y.size(3) and W == y.size(4)
...
if __name__ == "__main__":
    ...
    img0 = np.zeros((1,3,64,512,512))
    img0[...] = 255
    img1 = np.zeros((1,3,64,512,512))
    img1[:,0] = 255
    img2 = np.zeros((1,3,64,512,512))
    img2[:,1] = 255
    img3 = np.zeros((1,3,64,512,512))
    img3[:,2] = 255
    X = np.concatenate([img0,img1,img2,img3], axis=0)
    y = X.copy()
    X = torch.from_numpy(X)
    y = torch.from_numpy(y)
    Xm, ym = apply_mosaic_3d(X, y)
    Xm = Xm.numpy().astype("uint8").transpose(0, 2, 3, 4, 1)
    ym = ym.numpy().astype("uint8").transpose(0, 2, 3, 4, 1)
